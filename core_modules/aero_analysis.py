...
# Check if neuralfoil is installed
try:
    import neuralfoil as nf
except ImportError:
    print("\n" + "="*72)
    print("ERROR: NeuralFoil package not found")
    print("="*72)
    print("\nNeuralFoil is required for aerodynamic analysis.")
    print("\nInstallation options:")
    print("  1. Using pip:")
    print("     pip install neuralfoil")
    print("\n  2. Using conda:")
    print("     conda install -c conda-forge neuralfoil")
    print("\nAfter installation, run this script again.")
    print("\nFor more info: https://github.com/peterdsharpe/NeuralFoil")
    print("="*72 + "\n")
    sys.exit(1)
...
def load_dat_file(filepath: str) -> tuple[np.ndarray, dict]:
    ...
    # Parse estimated Reynolds from header if present
    for key in ('Estimated Reynolds Number', 'Est. Reynolds', 'estimated_reynolds'):
        if key in meta:
            try:
...
def run_simulation_for_file(
    ...
    # Extract bird name from metadata if available, otherwise from filename
    if 'Species' in file_meta:
        bird_name = file_meta['Species']
    else:
        # Fallback: extract from filename (e.g., airfoil_00421_Anorrhinus_galeritus_AR1.12)
        filename = Path(dat_file).stem
        parts = filename.split('_')
        # Try to find the species name (usually after the number)
        if len(parts) >= 3:
            # Join parts that look like species name (before AR notation)
            species_parts = []
            for i, part in enumerate(parts[2:], start=2):
                if part.startswith('AR'):
                    break
                species_parts.append(part)
            bird_name = '_'.join(species_parts) if species_parts else filename
        else:
            bird_name = filename
    
    category   = Path(dat_file).parent.name
    results    = []
...
def analyze_all_dat_files(
    ...
    Parameters
    ----------
    directory_path : str   Root directory containing .dat files (searched recursively).
    ...
    dat_files = sorted(glob.glob(
        os.path.join(directory_path, "**", "*.dat"), recursive=True
    ))
    ...
    for dat_file in tqdm(dat_files, desc="Simulating airfoils"):
        bird_name = Path(dat_file).stem

        try:
            file_results, file_meta = run_simulation_for_file(dat_file)
        except Exception as e:
            tqdm.write(f"  ✗ Failed to load {Path(dat_file).name}: {e}")
            continue
        ...
        category = Path(dat_file).parent.name
        summary  = build_summary(bird_name, category, dat_file, df_bird, file_meta)
    ...
    # ── Persist results ───────────────────────────────────────────────────
    detailed_df.to_csv(os.path.join(output_dir, "detailed_analysis.csv"), index=False)
    summary_df.to_csv(os.path.join(output_dir,  "summary_analysis.csv"),  index=False)

    with open(os.path.join(output_dir, "detailed_results.pkl"), 'wb') as f:
        pickle.dump(detailed_df.to_dict('records'), f)
    with open(os.path.join(output_dir, "summary_results.pkl"), 'wb') as f:
...
def analyze_category_leaders(
    ...
    with open(os.path.join(output_dir, "category_leaders.pkl"), 'wb') as f:
...
def create_visualizations(
    ...
    plt.suptitle('')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'ld_distribution_by_category.png'), dpi=300)
    plt.close()
    ...
    plt.suptitle('')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'clmax_by_category.png'), dpi=300)
    plt.close()
    ...
    plt.suptitle('')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'stall_angle_by_category.png'), dpi=300)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'top_performers.png'), dpi=300)
    plt.close()
    ...
    if re_cols:
        ...
        ax.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'ld_vs_reynolds.png'), dpi=300)
        plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'cl_alpha_polars.png'), dpi=300)
    plt.close()
...
if __name__ == "__main__":
    ...
    directory_path = str(PROJECT_ROOT / "OUTPUT" / "airfoils")
